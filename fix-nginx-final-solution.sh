...
# Final solution - use root with proper path handling

set -e

echo "🔧 Final Nginx solution..."
...
# The most reliable approach: Use root (not alias) and handle paths correctly
sudo node << 'NODE_EOF'
const fs = require('fs');
...
// Ojoto Union - use root with rewrite (most reliable)
...
if (ojotoApp) {
    ...
    // Use root (not alias) - set root to the dist directory
    // Then rewrite the path to remove the /ojoto-union prefix
    nginxConfig += '    location ' + frontendPath + '/ {\n';
    nginxConfig += '        root ' + ojotoDir.replace('/dist', '') + ';\n';
    nginxConfig += '        rewrite ^' + frontendPath + '/(.*)$ /dist/$1 break;\n';
    nginxConfig += '        try_files $uri $uri/ /dist/index.html;\n';
    nginxConfig += '    }\n\n';
...
console.log('Config regenerated with root + rewrite (no alias)');
NODE_EOF
...
if sudo nginx -t 2>&1; then
    ...
    if [ "$HTTP_CODE" = "200" ]; then
        ...
        curl -v http://localhost/ojoto-union/ 2>&1 | grep -E "(GET|HTTP|Location|404)" | head -5
        echo ""
        echo "Error log:"
