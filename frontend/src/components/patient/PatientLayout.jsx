  ...
  useEffect(() => {
    const fetchUnreadCount = async () => {
      if (user && user.patientId) {
        try {
          const response = await notificationService.getPatientUnreadCount(user.patientId);
          if (response.success) {
          ...
          <div className="flex flex-col h-full">
            ...
            <div className="p-6 border-b border-gray-200 bg-gradient-to-r from-primary to-primary-600">
              <Link to="/patient" onClick={() => setSidebarOpen(false)}>
                ...
                <p className="text-sm text-white/90 capitalize mt-1">Patient Portal</p>
              </Link>
            </div>
              ...
              <ul className="space-y-2">
                ...
                <li>
                  ...
                  >
                    ...
                    <FaBell className={`w-5 h-5 transition-all duration-200 ${
                      location.pathname === '/patient/notifications'
                        ? 'text-white'
                        : 'text-gray-500 group-hover:text-primary group-hover:scale-110'
                    }`} />
                    ...
                    {unreadCount > 0 && (
                      <span className={`flex items-center justify-center min-w-[20px] h-5 px-2 rounded-full text-xs font-semibold ${
                        location.pathname === '/patient/notifications'
